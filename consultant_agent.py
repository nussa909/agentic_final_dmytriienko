...
def consultant_planner(state: ConsultantState) -> dict:
    ...
    - Якщо потрібен розрахунок дозування або глибокий пошук протоколів — не намагайся зробити це сам! Останнім кроком плану вкажи: "Передати запит Супервізору для залучення Фармацевта/Дослідника".
...
def consultant_replanner(state: ConsultantState) -> dict:
    ...
    ПРАВИЛА ОЦІНКИ:
    1. Якщо план завершено і даних достатньо, поверни action='finish' та напиши фінальну відповідь для пацієнта (з медичним дисплеймером).
    2. Якщо ти розумієш, що запит стосується дозування (а це робота Фармацевта), поверни action='finish' і напиши у final_output: "Цей запит стосується дозування. Будь ласка, зверніться до цього ж чату, і наш Фармацевт допоможе вам." (Супервізор побачить це і перенаправить наступний запит).
    3. Інакше — 'continue' або 'replan'.
    """
